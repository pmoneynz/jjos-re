...
def write_report(path: Path, summary: dict[str, object]) -> None:
    ...
    lines = [
        ...
        f"- Legacy evidence count: `{best_base['mapped_word_count']}` aligned big-endian words map inside-file under that base arithmetic.",
    ...
    if capstone_info:
        lines.extend(
            [
                ...
                "",
                "- Optional Capstone probing produced valid M68k decodes in several early regions.",
                "- Treat this as historical context only; it is not the active architecture baseline for this repository.",
            ]
        )
